...
test.describe('Cellular Automata', () => {
  ...
  test('Patterns array is capped at 100 discovered patterns', async ({ page }) => {
    await page.evaluate(() => {
      for (let i = 0; i < 120; i++) {
        window.addDiscoveredPattern({ pop: 1, cells: [[0, 0], [1, 0]] });
      }
    });

    const len = await page.evaluate(() => PATTERNS.length);
